function computeMaxEditDistance(targetLowercase) {
    return Math.max(1, Math.ceil(targetLowercase.length / 3));
}
export function suggestSimilarWords(dictionary, rawWord, maxSuggestions = 5) {
    ...
    candidateResults.sort((a, b) => {
        ...
        if (pa === 0 && pb === 0) {
            ...
            if (aUncommon !== bUncommon)
                return aUncommon ? 1 : -1;
        }
...
function classifyEditPriority(target, candidate) {
    if (target === candidate)
        return 4;
    ...
    if (Math.abs(lt - lc) === 1) {
        if (isOneEditInsertOrDelete(target, candidate))
            return 1;
    }
    if (lt === lc) {
...
function isOneEditInsertOrDelete(a, b) {
    let i = 0, j = 0, edits = 0;
...
const KEY_ADJACENTS = new Map(Object.entries({
    q: ['w', 'a'],
    w: ['q', 'e', 'a', 's'],
    e: ['w', 'r', 's', 'd'],
    r: ['e', 't', 'd', 'f'],
...
function areKeyboardAdjacent(a, b) {
